...
export async function POST(req: Request) {
  ...
  try {
    ...
    const prompt = `
      ...
      【判定ルール】
      1. 入力が「特定の地域、観光、地元情報、歴史、グルメ、生活」など、地域情報まとめサイトのお題（テーマ）として成立するか判定。
      ...
      5. 解説文(ai_text): 
         - 200文字程度。できるだけ事実に基づき、このサイトを見ただけで満足できるように店名や地名など具体的な情報を盛り込んで作成。あとで人間が訂正してくれるので、些細な間違いは気にしなくて良いです。
